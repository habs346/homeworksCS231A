...
def compute_camera_matrix(real_XY, front_image, back_image):
    ...
    pass
    real_XY1 = np.hstack((real_XY, np.zeros((real_XY.shape[0], 1)), np.ones((real_XY.shape[0], 1))))
    real_XY2 = np.hstack((real_XY, 150*np.ones((real_XY.shape[0], 1)), np.ones((real_XY.shape[0], 1))))
    Pi=np.vstack((real_XY1,real_XY2))
    p=np.vstack((front_image,back_image))
    M1=np.linalg.lstsq(Pi,p)[0].T
    return np.vstack((M1,np.array([0,0,0,1])))
...
def rms_error(camera_matrix, real_XY, front_image, back_image):
    ...
    Pi=np.vstack((real_XY1,real_XY2))
    p=np.vstack((front_image,back_image))
    p_hat=np.dot(Pi,camera_matrix.T)
    p_hat=p_hat[:,:2]
    return np.sqrt(np.mean(np.sum((p-p_hat)**2,axis=1)))
    # END YOUR CODE HERE
